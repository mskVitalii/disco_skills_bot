...
## Миграции — ВСЕГДА через aerich, никогда руками

**Никогда не пиши и не редактируй файлы в `migrations/models/` вручную.** Каждый файл
миграции обязан содержать поле `MODELS_STATE` (сжатый снапшот состояния моделей) —
его добавляет только `aerich migrate`. Если создать/отредактировать файл руками
(или сгенерировать его чужой рукой без `MODELS_STATE`), при следующем деплое
свежий контейнер упадёт с:

```
RuntimeError: Old format of migration file detected, run `aerich fix-migrations` to upgrade format
```

Это происходит потому что `aerich init` (шаг перед `aerich upgrade`) проверяет
**последний по номеру** файл миграции и требует у него `MODELS_STATE`. Без него
она отказывается работать вообще — бот не поднимется, пока миграция не исправлена.

Правильный workflow при изменении моделей:
```bash
# 1. Поменял модели в app/models/*
# 2. Сгенерировал миграцию (это сравнивает текущие модели с последним MODELS_STATE
#    и само пишет корректный файл с новым MODELS_STATE)
uv run aerich migrate --name <короткое_описание>

# 3. Применил локально, чтобы проверить, что SQL корректен
uv run aerich upgrade
```

Если файл миграции уже потерял `MODELS_STATE` (например, кто-то его написал руками),
чинится так — но **только если** локальная БД содержит все применённые до этого
миграции:
```bash
uv run aerich fix-migrations
```
Если для сломанного файла нет соответствующей строки в таблице `aerich` (значит
эта миграция никогда не применялась через aerich, а SQL накатили руками) —
`fix-migrations` её пропустит с warning. Тогда сначала нужно руками добавить
строку в таблицу `aerich` с `content = get_models_describe("models")` (см.
`aerich.utils`), и только потом гонять `fix-migrations`.
